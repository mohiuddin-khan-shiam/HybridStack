"""
Bidirectional Long Short-Term Memory (BiLSTM) Optimization Pipeline.

This module provides a standalone, production-ready framework for constructing,
tuning, and training a stacked Bidirectional LSTM model with Optuna 
...
def create_bidirectional_lstm_model(trial, input_shape: tuple) -> Sequential:
    """Constructs and compiles a stacked BiLSTM model using Optuna suggestions.
    ...
    model = Sequential([
        ...
        Bidirectional(LSTM(units, activation=activation_function, return_sequences=True)),
...
class OptimizationObjective:
    ...
    def __call__(self, trial) -> float:
        model = create_bidirectional_lstm_model(trial, self.input_shape)
...
def run_pipeline(X_train_raw: np.ndarray, y_train_raw: np.ndarray, X_test_raw: np.ndarray, n_trials: int = 5):
    ...
    study = optuna.create_study(
        direction="minimize",
        study_name="BiLSTM_Optimization",
    ...
    best_model = create_bidirectional_lstm_model(
